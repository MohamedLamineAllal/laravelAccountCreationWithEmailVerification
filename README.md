...
UserCreatedProvider is created, and is used to bind and overide the `created` methode of the user Model. => when the user is created through this function we generate the verification token and we emit the event UserRegistered, so the listener will send the verification email.
...
We add the two routes for the  verification controller functionalities. One that will run `verify` method, and the other for `resend` method. 

**Mail sending**

To send the mail, we will be using laravel Mail system, the `mailable` class. we generate it using `php artisan make:mail SendVerificationToken`
...
Mass Assignement error: don't forget to set the fillable properties of you Model.
